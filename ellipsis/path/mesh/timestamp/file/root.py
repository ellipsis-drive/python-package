...
def add(pathId, timestampId, token, fileFormat=None, filePath=None, memFile = None, name = None):
    ...
    body = {'name':fileName, 'format':fileFormat}
    if type(filePath) != type(None):
        r = apiManager.upload('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file' , filePath, body, token)
    else:
        r = apiManager.upload('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file' , name, body, token, memfile = memFile)
...
def get(pathId, timestampId, token, pageStart= None, listAll = True):
    ...
    def f(body):
        r = apiManager.get('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file', body, token)
...
def trash(pathId, timestampId, fileId, token):
    ...
    r = apiManager.put('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file/' + fileId + '/trashed', {'trashed':True}, token)
    return r
...
def recover(pathId, timestampId, fileId, token):
    ...
    r = apiManager.put('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file/' + fileId + '/trashed', {'trashed':False}, token)
    return r
...
def delete(pathId, timestampId, fileId, token):
    ...
    r = apiManager.delete('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file/' + fileId, None, token)
    return r
...
def download(pathId, timestampId, fileId, filePath, token):
    ...
    apiManager.download('/path/' + pathId + '/mesh/timestamp/' + timestampId + '/file/' + fileId + '/data', filePath, token)
